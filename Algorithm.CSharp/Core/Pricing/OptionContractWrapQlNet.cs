...
namespace QuantConnect.Algorithm.CSharp.Core.Pricing
{
    ...
    public sealed class OptionContractWrapQlNet
    {
        ///<summary>
        ...
        private static readonly Dictionary<(Symbol, DateTime), OptionContractWrapQlNet> instances = new();
        ...
        private OptionContractWrapQlNet(Foundations algo, Securities.Option.Option contract, DateTime calculationDate)
        {
            _algo = algo;
        ...
        public static OptionContractWrapQlNet E(Foundations algo, Securities.Option.Option contract, DateTime calculationDate)
        {
            ...
            if (!instances.ContainsKey(singletonKey))
            {
                ...
                lock (instances)
                {
                    instances[singletonKey] = new OptionContractWrapQlNet(algo, contract, _calculationDate);
                }
            }
        ...
        internal double FiniteDifferenceApproxTime(Derive derive = Derive.NPV, int nDays = 1, Method method = Method.forward)
        {
            ...
            try
            {
                ...
                foreach (var dt in new List<Date> { calculationDate, calendar.advance(calculationDate, nDays, TimeUnit.Days) })
                {
                    ...
                    OptionContractWrapQlNet ocw1 = E(_algo, Contract, dt);
